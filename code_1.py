...
# ---------------------------
# Best practice: Apne PC me 'GEMINI_API_KEY' naam ka Environment Variable set karein.
# Agar environment variable nahi milta, toh tum yahan direct string me key daal sakte ho (not recommended for GitHub).
...
class PortScanner:
    ...
    def _grab_banner(self, s, port):
        ...
        try:
            # HTTP/HTTPS usually need a request first to send data back
            if port in [80, 443, 8080]:
                s.send(b"HEAD / HTTP/1.0\r\n\r\n")
            
            s.settimeout(1.0) # Short timeout for banner grabbing
            banner = s.recv(1024).decode('utf-8', errors='ignore').strip()
            # Keep banner short and clean
    ...
    def _scan_port(self, port):
        ...
        try:
            ...
            pass # Ignore connection errors to keep terminal clean
        finally:
            with self._lock:
                self.scanned_count += 1
                current_count = self.scanned_count
            # Update progress UI periodically to avoid freezing
    ...
    def run(self):
        # Using ThreadPoolExecutor for massive performance gain
...
class ScannerGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Replaced Text widget with Treeview for a cleaner look
    ...
    def start_scan(self):
        ...
        self.clear_results()
        self.scanner = PortScanner(target, start_port, end_port, timeout=0.5, max_workers=800)

        try:
            resolved_ip = self.scanner.resolve_target()
            self.title(f"Scanning: {target} ({resolved_ip})")
        except Exception as e:
        ...
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                ...
                for port, service, banner in sorted(self.scanner.open_ports, key=lambda x: x[0]):
                    # Clean commas from banner to avoid CSV breakage
    ...
    def _fetch_llm_analysis(self):
        # Now we send the banners to the AI as well!
        ...
        try:
            ...
            # NAYE PACKAGE KE HISAB SE GENERATION CODE
        ...
        try:
            while True:
                ...
                if msg_type == 'open':
                    ...
                    self.tree.insert("", "end", values=(port, service, banner))
...
def main():
    if sys.platform.startswith("win"):
        try:
            import ctypes
            ctypes.windll.shcore.SetProcessDpiAwareness(1) # Sharper UI on Windows
        except Exception:
            pass
    app = ScannerGUI()
    app.mainloop()
