...
# ---------------------------
# API Configuration (100% Secure)
# ---------------------------
# Yeh command .env file se automatically key read kar legi
load_dotenv() 
...
class ScannerGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.tree.tag_configure("high_risk", background="#ffcccc", foreground="black") 
        self.tree.tag_configure("medium_risk", background="#ffe6cc", foreground="black") 
        self.tree.tag_configure("secure", background="#ccffcc", foreground="black")      
        self.tree.tag_configure("neutral", background="white", foreground="black")       
        ...
        self.btn_save = ttk.Button(frm_bottom, text="Export Scan Data to CSV", command=self.save_results, state="disabled")
        self.btn_save.pack(side="right")

    def _get_risk_tag(self, port):
        high_risk_ports = [21, 23, 135, 139, 445, 3389]   
        medium_risk_ports = [80, 8080, 3306, 5900]        
        secure_ports = [22, 443]                          
    ...
    def start_scan(self):
        ...
        workers = speed_mapping.get(selected_speed, 800) 
        ...
        self.cmb_speed.configure(state="disabled") 
...
# -----------------------
    ...
    def _fetch_llm_analysis(self):
        ...
        prompt = (
            ...
            "Provide a concise, professional security assessment report. Point out critical vulnerabilities, "
            "evaluate the banners, and provide exact, bulleted mitigation steps."
        )
    ...
    def _show_report_window(self, report_text):
        ...
        rep_win.geometry("750x650") # Thoda bada kar diya taaki button fit aa jaye
        
        # Text Area for Report
        txt = tk.Text(rep_win, wrap="word", font=("Consolas", 10), bg="#1e1e1e", fg="#00ff00")
        txt.pack(fill="both", expand=True, padx=15, pady=(15, 5))
        txt.insert("1.0", report_text)
        txt.configure(state="disabled") 

        # --- NEW: Bottom Frame for Export Button ---
        btn_frame = ttk.Frame(rep_win)
        btn_frame.pack(fill="x", padx=15, pady=(0, 15))

        btn_export = ttk.Button(btn_frame, text="Export AI Report as TXT/MD", command=lambda: self._save_ai_report(report_text))
        btn_export.pack(side="right")

    # --- NEW: Function to save the AI Report ---
    def _save_ai_report(self, report_text):
        file_path = filedialog.asksaveasfilename(
            title="Export AI Report", 
            defaultextension=".txt",
            initialfile=f"AI_Security_Report_{int(time.time())}.txt",
        ...
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(report_text)
            messagebox.showinfo("Export Successful", "The AI Security Assessment has been saved successfully.")
        except Exception as e:
        ...
        try:
            while True:
                ...
                if msg_type == 'open':
                    ...
                    self.cmb_speed.configure(state="readonly") 
